<script lang="ts">
	import CardSpread from './CardSpread.svelte';
...
<div class="flex hero-portrait:flex-col justify-center items-center p-16 gap-6 {others.class}" style="--glow-color-1:{data.color}">
	...
	<div class="flex hero-portrait:flex-col gap-8">
		...
		<Break orientation="horizontal" class="hero-landscape:hidden"/>
		<Break orientation="vertical" class="hero-portrait:hidden"/>

		<div class="flex flex-col gap-1 max-w-xs hero-landscape:py-16">

			<div class="inline-block p-[2px] glow inverse-corners">
				<p class="text-base bg-surface text-center font-bold uppercase px-[2.5em] py-1 select-none inverse-corners">{data.name}</p>
			</div>

			<p class="italic text-base text-center">{data.description}</p>
